...
class APSExtractor(BaseExtractor):
    ...
    def extract(self, pdf_path: Path) -> Invoice:
        ...
        invoice_number = (
            self._find_pattern(text, r'Invoice\s+(?:No\.?|#)?\s*:?\s*(\d+)', re.IGNORECASE) or
            self._find_pattern(text, r'NO\.\s*(\d+)', re.IGNORECASE) or
            ""
        )

        # PO number - APS sometimes uses REF instead of PO
        po_number = (
            self._find_pattern(text, r'(?:Order|PO|P\.O\.)\s*(?:No\.?|#)?\s*:?\s*([A-Z0-9/]+)', re.IGNORECASE) or
            self._find_pattern(text, r'P/O\s+(?:No\.?|#)?\s*:?\s*([A-Z0-9/]+)', re.IGNORECASE) or
            ""
        )
        ...
        invoice_date = self.date_parser.parse_date(
            self._find_pattern(text, r'Invoice Date\s*:?\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE) or ""
        )

        # Extract store from install address or REF field
        store_location = ""
        store_match = re.search(r'(?:INSTALL ADDRESS|REF:)(.*?)(?:\n\n|$)', text, re.IGNORECASE | re.DOTALL)
        if store_match:
            store_text = store_match.group(1).strip()
            lines = [l.strip() for l in store_text.split('\n') if l.strip()]
        ...
        if not net_amount:
            net_amount = self.amount_parser.parse_amount(
                self._find_pattern(text, r'(?:Sub Total|Net)\s*:?\s*£?\s*([\d,]+\.?\d*)', re.IGNORECASE) or "0"
            )

        # Extract VAT
        vat_amount = self.amount_parser.parse_amount(
            self._find_pattern(text, r'VAT\s+@\s+\d+%\s+£\s*([\d,]+\.?\d*)', re.IGNORECASE) or
            self._find_pattern(text, r'VAT\s*:?\s*£?\s*([\d,]+\.?\d*)', re.IGNORECASE) or "0"
        )

        # Extract total (with VAT)
        total_amount = self.amount_parser.parse_amount(
            self._find_pattern(text, r'TOTAL\s+DUE\s+£\s*([\d,]+\.?\d*)', re.IGNORECASE) or
        ...
        invoice = Invoice(
            ...
            store_address=store_text if store_match else "",
