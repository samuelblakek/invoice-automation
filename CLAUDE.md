...
- **Fuzzy match can propose a wrong PO when the real one is absent** — if a PO isn't in the workbook at all (e.g. an older PO not carried into the current year's sheet), Strategy 3 fuzzy matching may still return a different PO. It's flagged by a low store-match score (lands in Review/Failed), but don't treat a fuzzy match as authoritative without checking the store.
